...
def reg_handle():
    ...
    if request.method == "GET":
        ...
        if (
            time.time()
            - db.reg_code.find_one({"email": email, "reg_code": verify_code})[
                "send_time"
            ]
            > 300
        ):  # 5 minutes
        ...
        if not re.fullmatch(
            r"[A-Za-z0-9\u4e00-\u9fa5]+@[a-zA-Z0-9_-]+(\.[a-zA-Z0-9_-]+)+", email
        ):
...
def post_anonymous_handle():
    ...
    if request.method == "GET":
        ...
        if content:
            if little_conponment.check_for_spam(content):
                return '<script>alert("NO SPAM！");history.back();</script>'
            ...
            if 0 < len(content) <= 1000:
                ...
                return '<script>alert("留言不能大於1000字或沒有字！");history.back();</script>'
        else:
...
def message_board_handle():
    ...
    if request.method == "GET":
        ...
        for document in messages:
            ...
            like_count = db.mb_reaction.count_documents(
                {"post_id": document.get("post_id"), "reaction": "like"}
            )
            dislike_count = db.mb_reaction.count_documents(
                {"post_id": document.get("post_id"), "reaction": "dislike"}
            )
            laugh_count = db.mb_reaction.count_documents(
                {"post_id": document.get("post_id"), "reaction": "laugh"}
            )

            resp_dict.append(
                (
                    document.get("uname"),
                    document.get("pub_time"),
                    content,
                    document.get("post_id"),
                    db.mb_replys.count_documents({"post_id": document.get("post_id")}),
                    document.get("might_fake"),
                    document.get("hidden"),
                    like_count,
                    dislike_count,
                    laugh_count,
        ...
        if content:
            ...
            if 0 < len(content) <= 1000:
                ...
                return '<script>alert("留言內容長度需在1-1000字之間！");history.back();</script>'
        else:
...
def messages_replys():
    ...
    if request.method == "GET":
        ...
        for document in messages:
            ...
            markdown_html_text = BeautifulSoup(
                markdown.markdown(document.get("content")), "html.parser"
            ).getText()
        ...
        if not user_info:
            return (
                '<script>alert("請先登入！");location.href="/login?after=messages_replys&post_id='
        ...
        if content and post_id:
            ...
            if user_info.get("uname") == db.mb_message.find_one(
                {"post_id": post_id}
            ).get("real_uname"):
                uname = "匿名"
            else:
            ...
            if 0 < len(content) <= 1000:
                db.mb_replys.insert_one(
                    {
                        ...
                        "hidden": False,
                        "might_fake": False,
                        "reply_id": little_conponment.generate_random_string(10)
                        + "reply"
                        + post_id,
                    }
                )
                ...
                return '<script>alert("回覆內容長度需在1-1000字之間！");history.back();</script>'
        else:
...
def login_handle():
    ...
    if request.method == "GET":
        if session.get("user_info"):
            if request.args.get("after"):
                if request.args.get("after") == "message_board":
                    ...
                    return redirect(
                        "/messages_replys?post_id=" + request.args.get("post_id")
                    )
        ...
        return Response(
            ...
            + "<script>if (location.search == '?reg=1') {alert('註冊成功！')}</script>"
        )
    elif request.method == "POST":
        upass = request.form.get("upass")
        uname = request.form.get("uname")
        ...
        try:
            isUsingOauth = db.mb_user.find_one({"uname": uname, "usingOauth": True})
            if isUsingOauth:
        ...
        try:
            ...
            if dbres:
                session["user_info"] = {
                    ...
                    "current_login_time": datetime.datetime.now().strftime(
                        "%Y-%m-%d %H:%M"
                    ),
                }
                db.mb_user.update_one(
                    {"uname": uname},
                    {"$set": {"last_login_time": datetime.datetime.now()}},
                )
            else:
