        ...
        for document in replys:
            ...
            replys_dict.append(
                (
                    ...
                    document.get("pub_time"),
                    content,
                    document.get("reply_id"),
                )
            )
        resp_replys = replys_dict

        return render_template(
            "mod_posts.html", messages=resp_messages, replys=resp_replys
        )
    ...
    if request.method == "POST":
        ...
        if str(request.json.get("action")) == "hide":
            ...
            if db.mb_replys.find_one({"reply_id": reply_id}).get("hidden"):
                db.mb_replys.update_one(
                    {"reply_id": reply_id}, {"$set": {"hidden": False}}
                )
                return jsonify({"err": 0, "desc": "已顯示！"})
            else:
                db.mb_replys.update_one(
                    {"reply_id": reply_id}, {"$set": {"hidden": True}}
                )
                return jsonify({"err": 0, "desc": "已隱藏！"})
            ...
            if db.mb_replys.find_one({"reply_id": reply_id}).get("might_fake"):
                db.mb_replys.update_one(
                    {"reply_id": reply_id}, {"$set": {"might_fake": False}}
                )
                return jsonify({"err": 0, "desc": "已取消標記！"})
            else:
                db.mb_replys.update_one(
                    {"reply_id": reply_id}, {"$set": {"might_fake": True}}
                )
                return jsonify({"err": 0, "desc": "已標記！"})
