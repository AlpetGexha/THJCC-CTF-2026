...
from app import app, db, limiter, mail
...
def index():
    """
    This is the thjcc-anon API v1
    """
    return "This is the thjcc-anon API v1"
...
def mb_board():
    ...
    for data in datas:
        ...
        info = {
            ...
            "might_fake": data["might_fake"],
            "hidden": data["hidden"],
            "like": db.mb_reaction.count_documents(
                {"post_id": data["post_id"], "reaction": "like"}
            ),
            "dislike": db.mb_reaction.count_documents(
                {"post_id": data["post_id"], "reaction": "dislike"}
            ),
            "laugh": db.mb_reaction.count_documents(
...
def mb_board_post():
    ...
    if data:
        ...
        replys = (
            db.mb_replys.find({"post_id": post_id})
            .limit(limit)
            .skip((page - 1) * limit)
        )
        for reply in replys:
...
def send_email_code():
    ...
    db.reg_code.insert_one(
        {"email": email, "reg_code": str(code), "send_time": time.time()}
    )
    # msg = Message("thjcc ANON 驗證碼", recipients=[email])
    # msg.body = "您的驗證碼是：" + str(code) + "，有效期為5分鐘。"
    # mail.send(msg)
    return jsonify({"err": 0, "desc": "驗證碼已發送！", "code":"您的驗證碼是：" + str(code) + " 有效期為5分鐘。"})
...
def reaction_api():
    ...
    """
    post_id = str(request.json.get("post_id"))
    reaction = str(request.json.get("reaction"))
    ...
    if db.mb_reaction.find_one({"post_id": post_id, "uname": user_info["uname"]}):
        try:
            ...
            db.mb_reaction.update_one(
                {"post_id": post_id, "uname": user_info["uname"]},
                {"$set": {"reaction": reaction}},
            )

    else:
        db.mb_reaction.insert_one(
            {"post_id": post_id, "uname": user_info["uname"], "reaction": reaction}
        )

    reactions = []
    for i in ["like", "dislike", "laugh"]:
        reactions.append(
            db.mb_reaction.count_documents({"post_id": post_id, "reaction": i})
        )
...
def upload_image():
    if not request.files:
        return jsonify({"err": 1, "desc": "no file uploaded!"})
    ...
    if not file.mimetype.startswith("image/"):
        return jsonify({"err": 1, "desc": "file is not an image!"})
    ...
    if app.config.get("DISALLOWED_EXTENSIONS", "") and secure_filename(filename).rsplit(
        ".", 1
    )[1].lower() in app.config.get("DISALLOWED_EXTENSIONS", ""):
        return jsonify({"err": 1, "desc": "file extension not allowed!"})
    ...
    return jsonify(
        {
            "err": 0,
            "desc": "success",
            "url": f"{request.host_url}/api/v1/get_image/{filename}",
        }
    )


# Return Uploaded image
@app.route("/api/v1/get_image/<filename>", methods=["GET"])
