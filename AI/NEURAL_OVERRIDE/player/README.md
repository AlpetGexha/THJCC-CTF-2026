# NEURAL_OVERRIDE (476) - Write-up

## Challenge Info
- Name: `NEURAL_OVERRIDE`
- Author: `kohiro`
- Category: AI / ML Security
- Target: `http://chal.thjcc.org:168/`
...
```json
{"confidence":"100.00%","flag":"THJCC{y0ur_ar3_the_adv3rs3r1al_attack_m0st3r}","status":"success"}
```
...
## Why This Works
The core bug is trusting serialized ML artifacts from users.  
In PyTorch, unsafe `torch.load` on attacker input is equivalent to remote code execution.

## Defensive Notes
- Never `torch.load` untrusted files in unsafe mode.
- Use safer formats (`safetensors`) where possible.
- If loading PyTorch files, enforce `weights_only=True` and strict type validation.
- Run model evaluation in isolated sandboxes with minimal privileges.
